...
M = {
    ...
    "w.ask.scoped": ("{n} 전용 위클리가 몇 % 이상 남았을 때만 실행할까요? (엔터 = 조건 없음): ",
                     "Run only when at least what % of the {n} scoped weekly remains? (Enter = none): "),
    ...
    "f.scoped": ("    모델 전용 한도 조건: 잔여 {v:g}% 이상", "    Model-scoped quota: ≥{v:g}% remaining"),
    ...
    "r.scoped.low": ("{n} 전용 위클리 잔여 {r:.0f}% < 조건 {m:g}%",
                     "{n} scoped weekly {r:.0f}% left < required {m:g}%"),
